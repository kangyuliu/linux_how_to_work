...
int main(int argc, char* argv[]) {
  ...
  progname = argv[0];  // 程序名字
  if (argc != 6) {
    fprintf(stderr,
            "usage: %s <filename> <kernel's help> <r/w> <access pattern> "
            "<block size[KB]>\n",
            progname);
    exit(EXIT_FAILURE);
  }
  ...
  // 参数2: 是否开启支援缓存
  bool help;
  if (!strcmp(argv[2], "on")) {  // 等于on，打开支援缓存
    help = true;
  } else if (!strcmp(argv[2], "off")) {  // 等于 off，关闭支援缓存
    help = false;
  } else {  // 其他，参数非法报错
    fprintf(stderr, "kernel's help should be 'on' or 'off': %s\n", argv[2]);
    exit(EXIT_FAILURE);
  }
  // 参数3: 访问类型，read or write
  int write_flag;
  if (!strcmp(argv[3], "r")) {
    write_flag = false;
  ...
  if (offset == NULL) err(EXIT_FAILURE, "malloc() failed");
  int flag = O_RDWR | O_EXCL;
  if (!help) flag |= O_DIRECT;
  int fd;
  ...
  for (i = 0; i < count; i++) {
    ...
    if (write_flag) {
      ...
      if (ret == -1) err(EXIT_FAILURE, "write() failed");
    } else {
      ret = read(fd, buf, block_size);
      if (ret == -1) err(EXIT_FAILURE, "read() failed");
    }
  }
  if (fdatasync(fd) == -1) err(EXIT_FAILURE, "fdatasync() failed");
  if (close(fd) == -1) err(EXIT_FAILURE, "close() failed");
  exit(EXIT_SUCCESS);
}